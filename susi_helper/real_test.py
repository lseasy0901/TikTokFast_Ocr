...
def test_4_get_machine_code():
    ...
    if 'pub mod fingerprint' in core_content:
        print("✓ susi_core exports fingerprint module")

        # Check if get_machine_code function exists
        if 'pub fn get_machine_code' in core_content:
            print("✓ get_machine_code function exists")
            print("  Function will be called via susi_helper.exe")
            return True
        else:
            print("⚠ WARNING: get_machine_code not found in exports")
            print("  Checking implementation...")
            if 'get_machine_code' in core_content:
                print("✓ Function is defined but may not be exported")
            else:
                print("⚠ WARNING: Function not found")
            return True
    else:
        print("⚠ WARNING: fingerprint module not found in exports")
        return True
...
def test_5_license_sign_verify():
    ...
    if all_found:
        print("✓ All license functions available for signing/verification")
    else:
        print("⚠ Some functions may be private")
...
def test_6_tamper_detection():
    ...
    print("✓ Signature verification should detect tampering")
    return True
...
def test_7_python_subprocess_integration():
    ...
    if result.returncode != 0:
        print(f"\n⚠ WARNING: Helper returned exit code {result.returncode}")
        if result.stderr:
            print(f"Helper stderr:")
            print(result.stderr)
    else:
        print("✓ Helper executed successfully")
        print("✓ JSON protocol working")
...
def test_8_keypath_verification():
    ...
    if 'susi_core' in cargo_content and 'path' in cargo_content:
        print("✓ Cargo.toml contains susi_core dependency")
        print("  This ensures local susi_core is used")
    else:
        print("⚠ WARNING: susi_core dependency may not be configured")
        return False
...
def clean_artifacts():
    ...
    if artifacts:
        ...
        for artifact in artifacts:
            print(f"  - {artifact}")
        print("\n⚠ SKIP: Manual cleanup recommended")
    else:
        print("\n✓ No test artifacts to clean")
